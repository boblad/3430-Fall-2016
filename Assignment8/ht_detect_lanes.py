...
def line_deg_angle(x1, y1, x2, y2):
    ...
    x = x2 - x1
    slope = y / x
    return (numpy.arctan(slope) * 180) / math.pi
...
def is_left_lane_line(x1, y1, x2, y2, ang_lower, ang_upper):
   angle = line_deg_angle(x1, y1, x2, y2)
   return is_angle_in_range(angle, ang_lower, ang_upper)

def is_right_lane_line(x1, y1, x2, y2, ang_lower, ang_upper):
    angle = line_deg_angle(x1, y1, x1, x2)
    return is_angle_in_range(angle, ang_lower, ang_upper)
...
def plot_ht_lanes_in_image(image_path, rho_accuracy, theta_accuracy, num_votes, min_len, max_gap):
    ...
    draw_lines_in_image(image, ll_lines, (255, 0, 0), 10)
    draw_lines_in_image(image, rl_lines, (0, 0, 255), 10)
